""" Environment wrappers. Great thanks to Timur Kamalbekov and Sergey Ivanov for refactoring this code. """
...
class ImagePreprocessing(gym.ObservationWrapper):
    ...
    def __init__(self, env, width=84, height=84, grayscale=True):
        ...
        low, high, dtype = 0, 1, np.float64#ospace.low.min(), ospace.high.max(), ospace.dtype
        if self.grayscale:
    ...
    def observation(self, observation):
        ...
        observation = cv2.resize(observation, (self.width, self.height),
                                 cv2.INTER_AREA)
        return observation / 255#observation
...
class _thunk:
    def __init__(self, i, env_seed, env_id):
        self.env_id = env_id
        self.i = i
        self.env_seed = env_seed

    def __call__(self, i=None, env_seed=None):
        env_seed = env_seed or self.env_seed
        return nature_dqn_env(self.env_id, seed=env_seed, summaries=False, clip_reward=False)
...
def nature_dqn_env(env_id, nenvs=None, seed=None,
    ...
    if nenvs is not None:
        ...
        wtf = [_thunk(i, env_seed, env_id) for i, env_seed in enumerate(seed)]
        env = ParallelEnvBatch(wtf)
